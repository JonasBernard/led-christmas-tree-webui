...
class CandleEffect(Effect):
    def __init__(self, adapter) -> None:
        ...
        self.started = False
        self.thread = threading.Thread(target=self.flicker_thread, args=[adapter, lambda: self.started])
    ...
    def flicker_thread(self, adapter, should_run_on):
        i = 1
        m = 1
        while should_run_on():
            i = i+1
            ...
            brightness = (math.sin(time.time() * frequency) + 1) / 2
